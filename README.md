...
2. **Install the Client Plugin:** Add the [OpenID Connect Generic](https://github.com/oidc-wp/openid-connect-generic) plugin to your `wp-content/plugins/` directory.
3. **Generate RSA Keys:** You need to [define RSA keys](https://github.com/Automattic/wp-openid-connect-server#define-the-rsa-keys) for secure token generation. Run the following in your terminal:
   ```bash
   openssl genrsa -out oidc.key 4096
   openssl rsa -in oidc.key -pubout -out public.key
   ```
4. **Add Keys to `wp-config.php`:** Copy the contents of those generated keys into your `wp-config.php` file:
   ```php
   ...
   ```
5. **Install the SSO mu-plugin:** Clone this repository directly into your Must-Use plugins folder:
   ```bash
   cd wp-content/mu-plugins
   git clone git@github.com:kingkool68/wordpress-super-simple-multisite-sso.git
   ```
6. **Configure:** Copy the `super-simple-multisite-sso.php.example` file into the root of your `wp-content/mu-plugins/` directory, rename it to `super-simple-multisite-sso.php`, and tweak the configuration options inside.
...
4. **The Global Goodbye:** When a user clicks "Log Out" anywhere on the network, the plugin orchestrates a global logout. It destroys the local session and pings the hub to terminate the master OpenID Connect session, ensuring the user is logged out across all mapped domains simultaneously.
